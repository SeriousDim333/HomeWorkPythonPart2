...
# представляет матрицу, где элементы разделены пробелами, а строки разделены символами новой строки. Например:
# 1 2 3
# 4 5 6
...
class Matrix:
    ...
    def __add__(self, other: "Matrix"):
        new_matrix = [[0]*self.cols for i in range(self.rows)]
        for i in range(len(self.data)):
            for j in range(len(self.data[i])):
                new_matrix[i][j] = self.data[i][j] + other.data[i][j]
        matrix_sum = Matrix(self.rows, self.cols)
        matrix_sum.data = new_matrix
        return matrix_sum
    
    def __mul__(self, other: "Matrix"):
        new_matrix = [[0]*other.cols for i in range(self.rows)]
        for i in range(len(self.data)):
            for j in range(len(other.data[0])):
                for k in range(len(other.data)):
                    new_matrix[i][j] += self.data[i][k]*other.data[k][j]
        matrix_mul = Matrix(self.rows, self.cols)
        matrix_mul.data = new_matrix
        return matrix_mul
